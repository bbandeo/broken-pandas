...
# TRATA COLUMNAS
df.columns = ["INFO", "COD", "EQUIV", "QTY", "TICK", "CLNT", "8020", "MES", "PESO", "ANCHO", "LARGO", "ALTO", "CD"]
...
df['CONTxMES'] = np.where(df['LIMITANTE'] == 'VOLUMEN', (df['QTY'] / df['CANTxVOL']) / df['MES'], (df['QTY'] / df['CANTxPESO']) / df['MES'])




# df.loc[df['CANTxPESO'] > df['CANTxVOL'],'VOLUMEN']
#  df.loc[df['CANTxPESO'] < df['CANTxVOL'],'PESO']

# SI LIMITANTE ES VOLUMEN THEN CANTIDAD / CAPxV
# SI LIMITANTE ES PESO THEN PESO / CAPxP
...
df_a3["LIMITANTE"] = np.where(df_a3["CANTxPESO"] > df_a3["CANTxVOL"], "VOLUMEN", "PESO")
df_a3["CONTxMES"] = np.where(df_a3["LIMITANTE"] == "VOLUMEN", (df_a3["QTY"] / df_a3["CANTxVOL"]) / df_a3["MES"], (df_a3["QTY"] / df_a3["CANTxPESO"]) / df_a3["MES"])
...
df_a2["LIMITANTE"] = np.where(df_a2["CANTxPESO"] > df_a2["CANTxVOL"], "VOLUMEN", "PESO")
df_a2["CONTxMES"] = np.where(df_a2["LIMITANTE"] == "VOLUMEN", (df_a2["QTY"] / df_a2["CANTxVOL"]) / df_a2["MES"], (df_a2["QTY"] / df_a2["CANTxPESO"]) / df_a2["MES"])
...
df_a1["LIMITANTE"] = np.where(df_a1["CANTxPESO"] > df_a1["CANTxVOL"], "VOLUMEN", "PESO")
df_a1["CONTxMES"] = np.where(df_a1["LIMITANTE"] == "VOLUMEN", (df_a1["QTY"] / df_a1["CANTxVOL"]) / df_a1["MES"], (df_a1["QTY"] / df_a1["CANTxPESO"]) / df_a1["MES"])



# df = df.loc[(df['ANCHO']> a3_ancho_gav) & (df['LARGO'] > a3_largo_gav)]
